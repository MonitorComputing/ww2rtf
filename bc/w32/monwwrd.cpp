...
MonWordWiseReader::MonWordWiseReader(
    ...
    poundSign('£'), hashChar('#'), printerTab(0xDD), padChar('|'),
    linesPerInch(6.0), charactersPerInch(10.0),
...
MonWordWiseReader::convert()
{
    ...
    {
        char    inputChar = (inputInt & 0xFF);

        if (!iscntrl(inputChar))
        {
            if (padChar == inputChar)
            {
                writer->writeHardSpace();
                traceAdd(' ');
            }
            else if (hashChar == inputChar)
            {
                writer->write(poundSign);
                traceAdd(poundSign);
            }
            else if (printerTab == inputChar)
            {
                writer->write('\t');
                traceAdd('\t');
            }
